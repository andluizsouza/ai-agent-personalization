...
class PlannerAgent:
    ...
    def _create_tools(self) -> List:
        ...
        @tool
        def run_analytical_query_tool(
            question: str, authenticated_client_id: str
        ) -> str:
            """
                ...
                self.execution_log.append(
                    {
                        ...
                        "execution_time_ms": execution_time,
                        "status": (
                            "success" if result.get("privacy_compliant") else "blocked"
                        ),
        ...
        def search_breweries_tool(
            ...
            """
            logger.info(
                f"Tool 2 called: search_breweries(city={city}, state={state}, type={brewery_type}, name={brewery_name}, filter={filter_history})"
            )
            start_time = time.time()
        ...
        @tool
        def get_website_summary_tool(
            brewery_name: str, url: str, brewery_type: str, address: str
        ) -> str:
            """
                ...
                self.execution_log.append(
                    {
                        ...
                        "timestamp": datetime.now().isoformat(),
                        "input": {"brewery_name": brewery_name, "url": url},
        ...
        try:
            ...
            if chat_history:
                for msg in chat_history[
                    :-1
                ]:  # Exclude the last message (it's in input)
    ...
    def get_metrics(self) -> Dict:
        ...
        tool3_calls = [
            log for log in self.execution_log if log["tool"] == "get_website_summary"
        ]
        cache_hits = sum(
            1 for log in tool3_calls if log.get("cache_status") == "CACHE_HIT"
        )
        ...
        return {
            ...
            "tools_breakdown": {
                "get_client_profile": len(
                    [l for l in self.execution_log if l["tool"] == "get_client_profile"]
                ),
                "search_breweries": len(
                    [
                        l
                        for l in self.execution_log
                        if l["tool"] == "search_breweries_by_location_and_type"
                    ]
                ),
                "get_website_summary": len(tool3_calls),
            },
            "cache_hit_rate": cache_hit_rate,
            "avg_tool_execution_time_ms": sum(
                l.get("execution_time_ms", 0) for l in self.execution_log
            )
