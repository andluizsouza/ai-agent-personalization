...
from agents import PlannerAgent, create_planner_agent
...
class ConversationalCLI:
    ...
    def request_client_id(self) -> bool:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                if attempt > 1:
                    console.print(
                        f"[yellow]Tentativa {attempt} de {max_attempts}[/yellow]\n"
                    )
                ...
                client_id = (
                    Prompt.ask("[bold cyan]Client ID[/bold cyan]").strip().upper()
                )
    ...
    def handle_command(self, command: str) -> bool:
        ...
        if command in ["/exit", "/quit"]:
            ...
            console.print(
                "[yellow]Digite /help para ver comandos disponíveis[/yellow]\n"
            )
    ...
    def show_chain_of_thought(self):
        ...
        console.print(
            Panel(
                "[bold]Chain-of-Thought - Última Execução[/bold]", border_style="cyan"
            )
        )
        ...
        for i, step in enumerate(self.agent.execution_log, 1):
            ...
            console.print(
                f"  Status: [{status_color}]{step['status']}[/{status_color}]"
            )
    ...
    def show_metrics(self):
        ...
        table = Table(
            title="Métricas da Sessão", show_header=True, header_style="bold magenta"
        )
        ...
        if isinstance(agent_metrics, dict) and "total_tool_calls" in agent_metrics:
            ...
            table.add_row(
                "Tool 1 (SQL Runner)",
                str(agent_metrics["tools_breakdown"]["get_client_profile"]),
            )
            table.add_row(
                "Tool 2 (Brewery Finder)",
                str(agent_metrics["tools_breakdown"]["search_breweries"]),
            )
            table.add_row(
                "Tool 3 (Web Explorer)",
                str(agent_metrics["tools_breakdown"]["get_website_summary"]),
            )
        ...
        while self.running:
            try:
                ...
                if response:
                    ...
                    if (
                        "sim" in response.lower()
                        and "não" in response.lower()
                        and "?" in response
                    ):
                        # This is likely the conditional question
                        console.print(
                            "[yellow]Dica: Responda 'sim' ou 'não' para continuar[/yellow]\n"
                        )
    ...
    def run(self):
        ...
        if not os.getenv("GOOGLE_API_KEY"):
            ...
            console.print(
                "[yellow] Configure a variável de ambiente GOOGLE_API_KEY[/yellow]"
            )
            sys.exit(1)
        ...
        if not self.client_id:
            if not self.request_client_id():
                ...
                console.print(
                    "[yellow] Até breve! Volte quando tiver seu Client ID.[/yellow]\n"
                )
                sys.exit(0)
        else:
            ...
            client_data = response["result"]
            console.print(f"[green]Cliente identificado com sucesso![/green]\n")
...
def main():
    ...
    parser.add_argument(
        "--client_id", type=str, help="Client ID pré-definido (ex: CLT-LNU555)"
    )
